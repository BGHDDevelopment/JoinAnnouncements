...
public class UpdateJoinEvent implements Listener
{
    ...
    public void onJoin(final PlayerJoinEvent e) {
    	...
    	if (p.hasPermission("join.update")) {
    		if (MainJoin.getPlugin().getConfig().getBoolean("Update.Enabled") == true){
    		this.checker = new UpdateChecker(MainJoin.plugin);
                        if (this.checker.isConnected()) {
                            if (this.checker.hasUpdate()) {
                                ...
                                p.sendMessage(ChatColor.RED + "Your version: " + MainJoin.plugin.getDescription().getVersion());
                                p.sendMessage(ChatColor.GRAY + "=========================");
                            }
                        }               
       }
